...
export default function FacultyDashboard() {
    ...
    const workspaceItems: WorkspaceItem[] = [
        { n: "Repository", i: BookOpenCheck, route: "/repository" },
        { n: "Review Uploads", i: ClipboardCheck, route: "/faculty/uploads" },
        { n: "Classes", i: Users, route: "/faculty/classes" },
        { n: "Assignments", i: ClipboardCheck, route: "/faculty/assignments" },
        { n: "Attendance", i: CalendarDays, route: "/faculty/attendance" },
                    ...
                    <div className="md:col-span-4 flex flex-col gap-4">
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-y-auto pr-1 space-y-3 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-primary/40">
                                {schedule.map((sch, i) => (
                                    <div key={i} className={`p-3.5 rounded-xl border-l-4 transition-all text-sm ${sch.done
                                        ...
                                        <div className="flex justify-between items-start gap-2 mb-2">
                                            <span className={`font-black text-sm leading-snug ${sch.done ? "line-through text-muted-foreground"
                            ...
                            <div className="grid grid-cols-4 gap-3">
                                {workspaceItems.map((item, idx) => (
                                    <div
                                        ...
                                        className={`tile flex flex-col items-center justify-center gap-2.5 cursor-pointer hover:shadow-md hover:-translate-y-0.5 hover:border-primary/50 transition-all group/tile !p-4 ${activeWorkspace === item.n ? "border-primary bg-primary/5 shadow-md" : ""
                                            }`}
                                    >
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center group-hover/tile:scale-110 transition-transform ${activeWorkspace === item.n
                                            ? "bg-primary text-primary-foreground"
                                            : "bg-accent border border-border text-foreground"
                                            }`}>
                                            <item.i className="w-5 h-5" strokeWidth={2.5} />
                                        </div>
                                        <span className={`font-black text-xs tracking-tight text-center ${activeWorkspace === item.n ? "text-primary" : "text-foreground"
                                            }`}>{item.n}</span>
                                    </div>
                                ))}
                            </div>
