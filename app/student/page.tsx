...
import {
  ...
  ChevronRight, BrainCircuit, Code, Search, Users
} from "lucide-react";
...
export default function StudentDashboard() {
  ...
  const workspaceItems: WorkspaceItem[] = [
    { n: "Repository", i: BookOpen, route: "/repository", color: "text-blue-500" },
          ...
          <div className="md:col-span-4 flex flex-col gap-4">
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto pr-1 space-y-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-primary/20">
                {schedule.map((sch, i) => (
                  <div key={i} className={`relative pl-4 transition-all duration-300 ${sch.done ? "opacity-50" : sch.active ? "opacity-100" : "opacity-80 hover:opacity-100"
                    }`}>
                    {/* Timeline Line */}
                    ...
                    <div className={`p-4 rounded-2xl border transition-all ${sch.active ? "bg-primary/5 border-primary/30 shadow-sm translate-x-1" : "bg-card border-border/50 hover:bg-accent hover:border-border"
                      }`}>
                      <div className="flex justify-between items-start gap-2 mb-2">
                        <div>
                          <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest mb-1.5 inline-block ${sch.type === 'Lecture' ? 'bg-blue-500/10 text-blue-500' :
